...
const WPLACE_BASE = 'https://backend-staging.wplace.live';
...
class WPlacer {
    ...
    async post(url, body) {
        const req = await this._fetch(url, {
            method: 'POST',
            headers: { Accept: '*/*', 'Content-Type': 'text/plain;charset=UTF-8', Referer: 'https://staging.wplace.live/' },
    ...
    function tryListen(idx = 0) {
        ...
        const server = app.listen(port, APP_HOST);
            ...
            if (!wsLogServer) {
                ...
                for (const { file } of logFilesToWatch) {
                    if (!existsSync(file)) {
                        writeFileSync(file, '', { flag: 'w' });
                    }
                }
                for (const { file, type } of logFilesToWatch) {
                    let lastSize = 0;
